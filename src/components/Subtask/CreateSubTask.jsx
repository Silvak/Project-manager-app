...
const CreateSubTask = ({ onClose, members }) => {
  const [projectId, index] = members.split(".");
  ...
  //CREATE subtask
  const addSubTask = (e, id, i) => {
    e.preventDefault();
    const projectToUpdate = projects.find((element) => element.id === id);
    const memosList = projectToUpdate.memos;
    ...
    i = parseInt(i);
    // Crear una copia profunda de memosList para su modificación
    const updatedMemosList = JSON.parse(JSON.stringify(memosList));

    // Asegurarse de que el array subTasks exista para el memo específico
    if (!updatedMemosList.list[i].subTasks) {
      updatedMemosList.list[i].subTasks = [];
    }

    // Simplemente añadir la nueva subtarea al array
    updatedMemosList.list[i].subTasks.push(payload);

    useUpdate("projects", id, {
      memos: {
        list: updatedMemosList.list,
      },
    });
    onClose();
  };
  ...
  return (
    <div className={styleModal.container}>
      <form
        onSubmit={(e) => addSubTask(e, projectId, index)}
        className={styleModal.form.container}
      >
        <h3 className={styleModal.form.title}>Crear Subtarea</h3>
